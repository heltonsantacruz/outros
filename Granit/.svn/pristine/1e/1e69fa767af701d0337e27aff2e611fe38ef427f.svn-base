...
<div class="titulo">
    <bean:message key="title.cadastro.produto"/>
</div>
<jsp:include page="/WEB-INF/comum/mensagens.jsp" />
<html:form focus="descricao" action="/produtoAction.event" >
	<script type="text/javascript">
            function adicionar() { 
                action = '<html:rewrite page="/produtoAction.event?operacao=adicionar"/>';
            ...
            function removerFornecedor(posicao) {                
                action = '<html:rewrite page="/produtoAction.event?operacao=removerFornecedor&posicao="/>' + posicao;
                document.forms[0].action = action;
                document.forms[0].submit();               
            }      

            function associar() { 
            	action = '<html:rewrite page="/produtoAction.event?operacao=associarFornecedor"/>';
                document.forms[0].action = action;
                document.forms[0].submit();        
            }    

            function atualizaSubTipos(){
             	action = '<html:rewrite page="/produtoAction.event?operacao=carregarSubTiposIncluir"/>';
                document.forms[0].action = action;
                document.forms[0].submit();   
            }
