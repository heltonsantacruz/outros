...
<div class="titulo">
    <bean:message key="title.alteracao.fornecedor"/>
</div>
...
<html:form focus="nome" action="/fornecedorAction.event" >
	<script type="text/javascript">
            ...
            function atualizar() { 
                action = '<html:rewrite page="/fornecedorAction.event?operacao=processarAlterar"/>';
                document.forms[0].action = action;
                document.forms[0].submit();               
            }
            
            function cancelar() { 
                action = '<html:rewrite page="/fornecedorAction.event?operacao=cancelarAlterar"/>';
                document.forms[0].action = action;
                document.forms[0].submit();               
            } 
                       
            function atualizaMunicipios(){
             	action = '<html:rewrite page="/fornecedorAction.event?operacao=carregarMunicipiosAlterar"/>';
                document.forms[0].action = action;
                document.forms[0].submit();   
            }

            function associar() { 
            	action = '<html:rewrite page="/fornecedorAction.event?operacao=associarProdutoAlteracao"/>';
                document.forms[0].action = action;
                document.forms[0].submit();        
            }
...
<display:table cellspacing="3" cellpadding="0" 
  ...
  <display:column title="" value="Remover" href="/granit/fornecedorAction.event?operacao=removerProdutoAlteracao" paramId="posicao" paramProperty="posicao"/>
